...
def hr_page():
    ...
    if st.button("Generate Response"):
        ...
        records = assistant_reply.split(";")
        records = records[1:]
        # Create an empty list to store the parsed data
        data = []

        # Iterate through each record and parse the information
        for record in records:
            ...
            for item in info:
                try:
                    ...
                    # Handle cases where the data cannot be split into key and value
                    pass
        ...
        assistant_table["Applied"] = random_values
...
def display_selected_rows_page():
    ...
    if selected_rows and not assistant_table.empty:
        for index in selected_rows:
            ...
            st.text_input(f"Name:", data[0], key=f"name_{index}")
            st.text_input(f"Email:", data[1], key=f"email_{index}")
            st.text_input(f"DOJ:", data[2], key=f"doj_{index}")
            st.text_input(f"WFH:", data[3], key=f"wfh_{index}")
            st.text_input(f"Dept:", data[4], key=f"dept_{index}")
